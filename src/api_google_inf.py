...
def _get_model_name(settings: Any, prefix: str) -> str:
    ...
    """
    return getattr(settings, f"google_{prefix}_model_name")

def _get_sampling_params(settings: Any, prefix: str) -> dict:
    """
    settingsオブジェクトとプレフィックスに基づき、Ollama用のサンプリングパラメータを生成する。
    """
    temperature = getattr(settings, f"google_{prefix}_temperature", 0.7)
    top_p = getattr(settings, f"google_{prefix}_top_p", 0.9)
...
def _execute_inference(model_name: str, prompts: List[str], options: dict, is_chat: bool) -> List[str]:
    ...
    """
    print(f"Google Geminiモデル '{model_name}' を使用して{len(prompts)}件の推論を実行中...")
    results = []
    try:
...
def inst_model_load(settings: Any) -> str:
    ...
    print(f"Google Geminiモデル '{model_name}' を使用します。")
    return model_name
...
def unload_model(model_path_for_log: str):
    print(f"Google Geminiモデル '{model_path_for_log}' のセッションを終了しました。")
    pass
